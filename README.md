...
**Deployment state — source and Production differ.** Progressive and split-pair acquisition are live in Production (split pairs since 2026-09-13, runbook §11h). Clear-HLS v1 is implemented in source and a retained release candidate has completed deterministic release-image qualification, but the currently running Production Worker has **not** been promoted to the HLS-capable source: HLS-10, the Production promotion, has not occurred, so HLS is not available in Production (runbook §4j).
...
yt-dlp is the generic HTTP/HTTPS extractor. The standalone Worker image ships a **pinned** yt-dlp runtime (exact release, digest-verified at build time, root-owned and read-only, no pip, no self-update), and user-supplied URLs reach it through the Worker's direct-first router: generic extraction was implemented in `PHASE-10C3-YTDLP-GENERIC-EXECUTION-INTEGRATION-001`, accepted live in Phase 10D, and is enabled in Production by `PHASE-10E-PERSISTENT-ON-DEMAND-GENERIC-ENABLEMENT-001`. Generic v1 is deliberately narrow — public, single-item, non-live sources acquired as one progressive HTTP(S) format, as a video-only + audio-only pair merged locally (deployed 2026-09-13, §11h), or, in current source only, through the clear-HLS v1 path, which VideoFetch acquires itself rather than yt-dlp (not yet deployed, §4j). There is no DASH acquisition and no HLS acquisition outside clear-HLS v1, so sources outside that scope yield no generic download option. See `docs/architecture/worker-deployment-runbook.md` §4g–§4h and §4j.
