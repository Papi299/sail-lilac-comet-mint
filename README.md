...
**Generic v1 scope.** Generic extraction covers public, single-item, non-live sources whose eligible media is available over progressive HTTP(S), either as one format or, for video presets, as a video-only + audio-only pair merged locally. It offers only application-owned presets — never raw upstream format ids. There is no HLS or DASH acquisition. Split renditions are therefore supported when both required streams satisfy the approved progressive HTTP(S) acquisition boundary; sources whose usable renditions require HLS or DASH still yield no generic video option. This capability was deployed to Production on 2026-09-13 (`docs/architecture/worker-deployment-runbook.md` §11h).
