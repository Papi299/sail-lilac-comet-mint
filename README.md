...
**Generic v1 scope.** Generic extraction covers public, single-item, non-live sources that can be acquired as one progressive HTTP(S) format, and it offers only application-owned presets — never raw upstream format ids. There is no HLS or DASH acquisition. Split renditions are supported: a video-only rendition may be paired with an audio-only one and merged locally, so such sources can still offer video presets — deployed to Production on 2026-09-13 (`docs/architecture/worker-deployment-runbook.md` §11h). A source whose usable renditions would need HLS or DASH acquisition still yields no generic video option.
