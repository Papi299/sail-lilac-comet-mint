...
`WORKER-LIVENESS-STATIC-USER-CORRECTION-001`** (its documentation part
re-measured nothing; it records the operator-held evidence of
`WORKER-EXTERNAL-LIVENESS-TLS-HEALTH-LIVE-ACCEPTANCE-001`). The previous records
were `HLS-10-PRODUCTION-ACCEPTANCE-DOCS-CLOSURE-001` and, before it,
`HLS-PRE-HLS10-DOCS-STATE-RECONCILIATION-001` (both earlier on 2026-09-26). The Worker
